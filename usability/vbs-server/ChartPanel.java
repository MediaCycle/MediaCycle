...
public class ChartPanel extends JPanel {
  ...
  public void paintComponent(Graphics g) {
    ...
    g.drawString(sb.toString(), 14, 45);
	...
	for (int i=0; i < VBSServer.TEAMS; i++) {
		...
		for (int k=0; k < VBSServer.SUBMISSION[i].size(); k++) {    	
			...
			if (sub.video.length() == 1)
				sub.video = "0" + sub.video;
			String filename = sub.video + "small" + VBSServer.DELIM + "frame" + sub.start + ".jpg";
			int barHeight = 60;
			...
			try {
				...
				if (sub.video != null && sub.video.length() > 0) {
