...
/*
 - FUNCTION: AREA
 - FUNCTIONALITY: Gives the total area of the blobs
 - PARAMETERS:
 - RESULT:
 - RESTRICTIONS:
...
/*
 ...
 - FUNCTIONALITY: Gives the bounding box around all the blobs
 - PARAMETERS:
 - RESULT:
 - RESTRICTIONS:
 - AUTHOR: Xavier Siebert
 ...
 - MODIFICATION: Date. Author. Description.
 */
CvRect CBlobResult::GetBoundingBox()
{
	double big_number = 10000;
	double minx = big_number;
	...
	for(int i=0; i<GetNumBlobs(); i++){
		if (minx > m_blobs[i]->MinX() ) minx =  m_blobs[i]->MinX();
		if (miny > m_blobs[i]->MinY() ) miny =  m_blobs[i]->MinY();
		if (maxx < m_blobs[i]->MaxX() ) maxx =  m_blobs[i]->MaxX();
...
CvPoint CBlobResult::GetCenter(){
	// XS TODO: check if already computed
	CvRect bbox = GetBoundingBox();
	CvPoint center;
	center.x = bbox.x + 0.5 * bbox.width;
	center.y = bbox.y + 0.5 * bbox.height;
	return center;	
}
