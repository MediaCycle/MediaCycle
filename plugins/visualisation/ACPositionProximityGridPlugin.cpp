...
ACPositionProximityGridPlugin::ACPositionProximityGridPlugin() : ACFilteringPlugin() {
    this->mMediaType = MEDIA_TYPE_ALL; // ALL
    // this->mPluginType = PLUGIN_TYPE_NONE;
    this->mName = "Proximity Grid (old)";
    ...
    this->addCallback("Update","Update view",boost::bind(&ACPositionProximityGridPlugin::setProximityGrid,this));
    /*this->addNumberParameter("Border: left",proxgridl,-1,0,0.1,"Border: left");//,boost::bind(&ACPositionProximityGridPlugin::setProximityGrid,this));
    this->addNumberParameter("Border: right",proxgridr,0,1,0.1,"Border: right");//,boost::bind(&ACPositionProximityGridPlugin::setProximityGrid,this));
    this->addNumberParameter("Border: top",proxgridt,-1,0,0.1,"Border: top");//,boost::bind(&ACPositionProximityGridPlugin::setProximityGrid,this));
    this->addNumberParameter("Border: bottom",proxgridb,0,1,0.1,"Border: bottom");//,boost::bind(&ACPositionProximityGridPlugin::setProximityGrid,this));*/
}
...
void ACPositionProximityGridPlugin::filter() {
    if(!media_cycle)
        return;
    mediaBrowser = media_cycle->getBrowser();
...
void ACPositionProximityGridPlugin::setProximityGrid() {
    ...
    //CF
    this->proxgridboundsset = 1;
    /*this->proxgridl = this->getNumberParameterValue("Border: left");
    this->proxgridr = this->getNumberParameterValue("Border: right");
    this->proxgridb = this->getNumberParameterValue("Border: botton");
    this->proxgridt = this->getNumberParameterValue("Border: top");*/
            ...
            for (j=0;j<2;j++) {
                for (k=0;k<l;k++) {
                    ...
                    if ( (curpos.x<0) || (curpos.x>=proxgridlx) || (curpos.y<0) || (curpos.y>=proxgridly) ) {
                        ...
                        if (curposindex >= 0 && proxgrid[curposindex] && proxgrid[curposindex]==-1) {
                            l=proxgridmaxdistance;
                            found_slot = 1;
                            k=l;
                            j=2;
