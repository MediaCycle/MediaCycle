...
static double compute_distance(vector<ACMediaFeatures*> &obj1, const vector<FeaturesVector> &obj2, const vector<float> &weights, bool inverse_features)
{
    ...
    if( (obj1.size() != obj2.size()) || (obj1.size() != weights.size())){
        //std::cerr << "ACChildKMeansPlugin::compute_distance: obj1.size() " << obj1.size() << " obj2.size() " << obj2.size() << " weights.size() " << weights.size() << std::endl;
        return dis;
    }
...
ACChildKMeansPlugin::ACChildKMeansPlugin()
: ACClusterMethodPlugin() {
    this->mMediaType = MEDIA_TYPE_ALL;
    this->mName = "MediaCycle Child KMeans";
    this->mDescription = "Plugin for KMeans Clustering of only child nodes";
    this->mId = "";
    this->addNumberParameter("clusters",5,1,10,1,"number of desired clusters",boost::bind(&ACChildKMeansPlugin::clusterNumberChanged,this));
}
...
void ACChildKMeansPlugin::updateClusters(ACMediaBrowser* mediaBrowser,bool needsCluster){
    ...
    for (ACMediaNodes::const_iterator it=mediaBrowser->getMediaNodes().begin(); it !=mediaBrowser->getMediaNodes().end();it++){
        if(it->second){
            ...
            if ((library->getMedia(it->first)->getMediaType() == library->getMediaType()) &&(library->getMedia(it->first)->getParentId()!=-1)&&(it->second->getNavigationLevel() >= mediaBrowser->getNavigationLevel()))
...
void ACChildKMeansPlugin::clusterNumberChanged(){
