...
#include "ACPlugin.h"
#include "ACVisPluginSne.h"
#include "Sne.h"

using namespace arma;
using namespace std;
//TR: I modified this class to take into account the feature that are selected by the user (with te weights).
ACVisPluginSne::ACVisPluginSne() : ACClusterPositionsPlugin()
{
    //vars herited from ACPlugin
    this->mMediaType = MEDIA_TYPE_ALL;
    this->mName = "MediaCycle Sne";
    this->mDescription = "dimensionnality reduction resulting from Sne";
    this->mId = "";
    this->addNumberParameter("perplexity",30,5,50,5,"perplexity");

    //local vars
}

ACVisPluginSne::~ACVisPluginSne(){
}


void ACVisPluginSne::updateNextPositions(ACMediaBrowser* mediaBrowser){
   ...
   // mat descN_m = zscore(desc_m);
    ...
    Sne Sne_algo;
    Sne_algo.setFeatureMatrix(desc_m);
    posDisp_m=Sne_algo.compute(2);
    ...
    if (cpt!=desc_m.n_rows)
        cout << "ACVisPluginSne::updateNextPositions, problem with desc matrix dimensions "<<endl;
    ////////////////////////////////////////////////////////////////
}


void ACVisPluginSne::extractDescMatrix(ACMediaBrowser* mediaBrowser, mat& desc_m, vector<string> &featureNames){
    ...
    if (nbFeature!=weight.size())
        std::cerr<<"ACVisPluginSne::extractDescMatrix weight vector size incompatibility"<<endl;
