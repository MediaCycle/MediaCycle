...
ACVisPluginSMDS::ACVisPluginSMDS() : ACClusterPositionsPlugin()
{
    //vars herited from ACPlugin
    this->mMediaType = MEDIA_TYPE_ALL;
    this->mName = "MediaCycle S-MDS";
    this->mDescription = "dimensionnality reduction resulting from Supervized MDS";
    this->mId = "";

    //local vars
}

ACVisPluginSMDS::~ACVisPluginSMDS(){
}


void ACVisPluginSMDS::updateNextPositions(ACMediaBrowser* mediaBrowser){
   ...
   // mat descN_m = zscore(desc_m);
    ...
    algo.setFeatureMatrix(desc_m,label);
    ...
    if (cpt!=desc_m.n_rows)
        cout << "ACVisPluginSMDS::updateNextPositions, problem with desc matrix dimensions "<<endl;
    ////////////////////////////////////////////////////////////////
}


void ACVisPluginSMDS::extractDescMatrix(ACMediaBrowser* mediaBrowser, mat& desc_m, vector<string> &featureNames){
    ...
    if (nbFeature!=weight.size())
        std::cerr<<"ACVisPluginSMDS::extractDescMatrix weight vector size incompatibility"<<endl;
