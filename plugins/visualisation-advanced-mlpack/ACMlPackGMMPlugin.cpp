...
ACMlPackGMMPlugin::ACMlPackGMMPlugin()
: ACClusterMethodPlugin() {
    this->mMediaType = MEDIA_TYPE_ALL;
    this->mName = "MediaCycle MlPack GMM";
    this->mDescription = "Plugin for GMM Clustering by using MLPACK";
    this->mId = "";
    this->addNumberParameter("clusters",5,1,10,1,"number of desired clusters",boost::bind(&ACMlPackGMMPlugin::clusterNumberChanged,this));
}
...
void ACMlPackGMMPlugin::updateClusters(ACMediaBrowser* mediaBrowser,bool needsCluster){
    ...
    if(library == 0) {
        cerr << "<ACMlPackGMMPlugin::updateClusters> : Media Library 0" << endl;
        return;
    }
    else if(library->isEmpty()) {
        cerr << "<ACMlPackGMMPlugin::updateClusters> : empty Media Library " << endl;
        return;
    }
...
void ACMlPackGMMPlugin::extractDescMatrix(ACMediaBrowser* mediaBrowser, mat& desc_m, vector<string> &featureNames,urowvec &tag){
    ACMedias medias = mediaBrowser->getLibrary()->getAllMedia();
    std::vector<long> ids = mediaBrowser->getLibrary()->getAllMediaIds();
    int nbMedia = medias.size();
    int featDim;
    ...
    if (nbFeature!=weight.size())
        cerr<<"ACArmaVisPlugin::extractDescMatrix weight vector size incompatibility"<<endl;
...
void ACMlPackGMMPlugin::transferToClusterCenter(ACMediaBrowser* mediaBrowser,  const arma::mat& center){
    ACMediaLibrary *library=mediaBrowser->getLibrary();
...
void ACMlPackGMMPlugin::clusterNumberChanged(){
