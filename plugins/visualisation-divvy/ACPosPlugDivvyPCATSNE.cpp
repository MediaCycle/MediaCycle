...
#include<armadillo>
#include "ACPosPlugDivvyPCATSNE.h"

using namespace arma;

ACPosPlugDivvyPCATSNE::ACPosPlugDivvyPCATSNE() : ACClusterPositionsPlugin() {
    this->mMediaType = MEDIA_TYPE_ALL;
    this->mName = "Divvy TSNE with PCA initialization";
    this->mDescription = "Plugin for positioning items using Divvy's tSNE dimensionality reduction with PCA initialization";
    this->mId = "";
...
ACPosPlugDivvyPCATSNE::~ACPosPlugDivvyPCATSNE() {
}

void ACPosPlugDivvyPCATSNE::updateNextPositions(ACMediaBrowser* mediaBrowser){
    ...
    if(libSize != ids.size())
        std::cout << "ACPosPlugDivvyPCATSNE::updateNextPositions: warning, lib size and number of accessible media ids don't match"<< std::endl;
    ...
    perform_tsne(desc, desc_m.n_cols, desc_m.n_rows, pos, 2, perplexity, (int)max_iter, 0);
    ...
    if(pos){//pos.n_rows == ids.size() && pos.n_cols == 2){
        ...
        for (int i=0; i<ids.size(); i++){
            ...
            //std::cout << "ACPosPlugDivvyPCATSNE::updateNextPositions: media " << ids[i] << " x " << p.x << " y " << p.y << std::endl;
        }
    }
    free(pos);
    free(desc);
...
void ACPosPlugDivvyPCATSNE::extractDescMatrix(ACMediaBrowser* mediaBrowser, arma::mat& desc_m, vector<string> &featureNames){
    ...
    if (nbFeature!=weight.size())
        std::cerr<<"ACPosPlugDivvyPCATSNE::extractDescMatrix weight vector size incompatibility"<<endl;
