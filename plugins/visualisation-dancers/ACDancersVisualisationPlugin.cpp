...
ACDancersVisualisationPlugin::ACDancersVisualisationPlugin() : ACClusterPositionsPlugin()
{
    ...
    this->mName = "MediaCycle DANCERS!";
    this->mDescription = "Positioning plugin for Bud Blumenthal's Bud Hybrid DANCERS! project";
    this->mId = "";
...
void ACDancersVisualisationPlugin::updateNextPositions(ACMediaBrowser* mediaBrowser){
    ...
    int itemClickedId = mediaBrowser->getClickedNode();
    int labelClickedId = mediaBrowser->getClickedLabel();

    std::vector<long>::iterator itemClickedIter = std::find(ids.begin(),ids.end(),itemClickedId);
    if(itemClickedIter!=ids.end())
        itemClicked = std::distance(ids.begin(),itemClickedIter);
    std::vector<long>::iterator labelClickedIter = std::find(ids.begin(),ids.end(),labelClickedId);
    if(labelClickedIter!=ids.end())
        labelClicked = std::distance(ids.begin(),labelClickedIter);
    ...
    if (itemClicked >=0) {
        posDisp_m(itemClicked,0) = 500;
        posDisp_m(itemClicked,1) = 500;
    }
    ...
    mediaBrowser->setNumberOfDisplayedLabels(labelIdx_v.n_rows);
    mediaBrowser->setNumberOfDisplayedNodes(toDisplay_v.n_elem);
    ////////////////////////////////////////////////////////////////////////////////////
    ...
    for (int i=0; i<libSize; i++){
        ...
        mediaBrowser->setMediaNodeDisplayed(ids[i], false);
    }
    ////////////////////////////////////////////////////////////////

    for (int i=0; i < toDisplay_v.n_elem; i++)
        mediaBrowser->setMediaNodeDisplayed(ids[toDisplay_v(i)], true);

    for (int i=0; i<mediaBrowser->getNumberOfDisplayedNodes(); i++){
        //TODO make sur you meant next
        p.x = posDisp_m(toDisplay_v(i),0);
        p.y = posDisp_m(toDisplay_v(i),1);
        p.z = 0;
        mediaBrowser->setNodeNextPosition(ids[toDisplay_v(i)], p);
    }
    // 	if (itemClicked >= 0)
    //TODO make sur you meant next
...
mat ACDancersVisualisationPlugin::updateNextPositionsItemClicked3(mat &desc_m, int nbVideoDisplay, int itemClicked, ucolvec &toDisplay_v, mat &labelPos_m, ucolvec &labelIdx_v, colvec &labelValue_v){
    ...
    mat labelPosRand_m = randn<mat>(3,2)*.05;
    ...
    labelPos_m = labelPosRand_m + labelPos_m * .6;
...
mat ACDancersVisualisationPlugin::extractDescMatrix(ACMediaBrowser* mediaBrowser, int nbActiveFeatures, vector<string> &featureNames, mat &descD_m){
    ...
    for(int i=0; i<nbMedia; i++) {
        ...
        for(int f=0; f< activeFeatures_v.n_elem; f++){
            featDim = mediaBrowser->getLibrary()->getFirstMedia()->getPreProcFeaturesVector(activeFeatures_v(f))->getSize();
            featureNames.push_back(mediaBrowser->getLibrary()->getFirstMedia()->getPreProcFeaturesVector(activeFeatures_v(f))->getName());
            for(int d=0; d < featDim; d++){
                desc_m(i,tmpIdx) = medias[ids[i]]->getPreProcFeaturesVector(activeFeatures_v(f))->getFeatureElement(d);
                descD_m(i,tmpIdx) = medias[ids[i]]->getPreProcFeaturesVector(activeFeatures_v(f))->getDiscretizedFeature();
                tmpIdx++;
            }
        }
    }
