/*
 *  ACAudioKISOsgRendererPlugin.cpp
 *  MediaCycle
 *
...
#include "ACAudioKISOsgRendererPlugin.h"
...
ACAudioKISOsgRendererPlugin::ACAudioKISOsgRendererPlugin() : ACOsgRendererPlugin(){
    this->mName = "Audio Renderer (OSG and libsndfile)";
    this->mDescription ="Plugin for rendering audio files with OpenSceneGraph for KIS tasks";
...
ACAudioKISOsgRendererPlugin::~ACAudioKISOsgRendererPlugin(){
}

std::map<std::string,ACMediaType> ACAudioKISOsgRendererPlugin::getSupportedExtensions(ACMediaType media_type){
...
std::vector<ACMediaType> ACAudioKISOsgRendererPlugin::getSupportedMediaTypes(){
    std::vector<ACMediaType> media_types;
    media_types.push_back(MEDIA_TYPE_AUDIO);
    return media_types;
}

ACOsgMediaRenderer* ACAudioKISOsgRendererPlugin::createMediaRenderer(ACMediaType media_type){
    ACOsgMediaRenderer* renderer = 0;
    if(media_type == MEDIA_TYPE_AUDIO){
        renderer = new ACOsgAudioRenderer();
        ((ACOsgAudioRenderer*)renderer)->setWaveformType(AC_BROWSER_AUDIO_WAVEFORM_NONE);
    }
    return renderer;
}
