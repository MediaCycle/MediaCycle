...
ACVisPluginPCA13::ACVisPluginPCA13() : ACArmaVisPlugin()
{
    ...
    this->mDescription = "axes 1 and 3 resulting from PCA visualisation";
    this->mId = "";
...
void ACVisPluginPCA13::updateNextPositions(ACMediaBrowser* mediaBrowser){
    ...
    float my1=abs(min(posDisp_m.row(2)));
    float my2=max(posDisp_m.row(2));
    ...
    for (long i=0; i<ids.size(); i++){
        //mediaBrowser->setMediaNodeDisplayed(ids[i], true);
        // TODO: make sure you meant next
	p.x = posDisp_m(i,0)/mTot;
	p.y = posDisp_m(i,2)/mTot;
        p.z = 0;
        mediaBrowser->setNodeNextPosition(ids[i], p);
    }
    ////////////////////////////////////////////////////////////////
...
void  ACVisPluginPCA13::dimensionReduction(mat &posDisp_m,arma::mat desc_m,arma::umat tag){
    mat tempPosDisp;
    mat descN_m = zscore(desc_m);
    mat coeff;
    mat score;
