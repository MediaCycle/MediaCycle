/**
 * @brief ACVisPlugin_constrained_t_Sne.cpp
...
#include "ACVisPlugin_constrained_t_Sne.h"
#include "constrained_t_Sne.h"

using namespace arma;
using namespace std;
//TR: I modified this class to take into account the feature that are selected by the user (with te weights).
ACVisPlugin_constrained_t_Sne::ACVisPlugin_constrained_t_Sne() : ACArmaVisPlugin(){
    //vars herited from ACPlugin
    this->mMediaType = MEDIA_TYPE_ALL;
    this->mName = "MediaCycle constrained t_Sne";
    this->mDescription = "dimensionnality reduction resulting from t_Sne";
    this->mId = "";
    perplexity=20;
    this->addNumberParameter("perplexity",perplexity,10,100,10,"value of desired perplexity",boost::bind(&ACVisPlugin_constrained_t_Sne::perplexityValueChanged,this));
    itMax=500;
    this->addNumberParameter("Iteration Number",itMax,10,2000,50,"Number of t-Sne iteration",boost::bind(&ACVisPlugin_constrained_t_Sne::itMaxValueChanged,this));
    vector<string> initializeValue;
    initializeValue.push_back("Yes");
    initializeValue.push_back("No");
    this->addStringParameter("Initialize","No",initializeValue,"Must we catch the current positions to initialize the t-Sne algorithm",boost::bind(&ACVisPlugin_constrained_t_Sne::initValueChanged,this));
    initFlag=false;
    
    //local vars
}

void ACVisPlugin_constrained_t_Sne::mediaCycleSet()
{
    // CF this will work only since/if we load feature plugins before this plugin...
    this->updateAvailableFeatures();
}
void ACVisPlugin_constrained_t_Sne::assignedFeaturesChanged(){
    if(!this->media_cycle) return;
    media_cycle->updateDisplay(true);
    media_cycle->setNeedsDisplay(true);
}

bool ACVisPlugin_constrained_t_Sne::updateAvailableFeatures(){
    if(this->media_cycle){
        if(this->media_cycle->getPluginManager()){
            ...
            if(feature_names.size()>=1){
                if(this->hasStringParameterNamed("x"))
                    ...
                    this->addStringParameter("x",feature_names.front(),feature_names,"feature assigned to x axis",boost::bind(&ACVisPlugin_constrained_t_Sne::assignedFeaturesChanged,this));
                return true;
            }
        }
    }
...
ACVisPlugin_constrained_t_Sne::~ACVisPlugin_constrained_t_Sne(){
}


void ACVisPlugin_constrained_t_Sne::perplexityValueChanged(void){
    if(!this->media_cycle) return;
    
    perplexity=this->getNumberParameterValue("perplexity");
    
}
void ACVisPlugin_constrained_t_Sne::itMaxValueChanged(void){
    if(!this->media_cycle) return;
    
    itMax=this->getNumberParameterValue("Iteration Number");
    
}
void ACVisPlugin_constrained_t_Sne::initValueChanged(void){
    if(!this->media_cycle) return;
...
void  ACVisPlugin_constrained_t_Sne::dimensionReduction(mat &posDisp_m,arma::mat desc_m,arma::umat tag){
    constrained_t_Sne t_Sne_algo;
    ...
    if (initFlag){
        ...
        tempMat=t_Sne_algo.compute(1,pos_m);
    }
    else
        tempMat=t_Sne_algo.compute(1);
    arma::colvec tempVec=t_Sne_algo.getX();
...
void ACVisPlugin_constrained_t_Sne::updateNextPositions(ACMediaBrowser* browser){
