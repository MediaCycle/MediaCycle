...
void  ACVisPluginSIsoMap::dimensionReduction(mat &posDisp_m,arma::mat desc_m,arma::umat tag){
    sIsomap algo;
    urowvec _tag=tag.col(0).t();
    while (algo.setFeatureMatrix(desc_m,_tag,'k',Kn)==false){
        Kn=Kn+5;
        cout<<"ACVisPluginIsoMap::updateNextPositions Kn:"<<Kn<<endl;
        if (Kn>desc_m.n_rows)
            break;
