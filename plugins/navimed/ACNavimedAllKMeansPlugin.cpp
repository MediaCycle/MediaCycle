...
static double compute_distance(vector<ACMediaFeatures*> &obj1, vector<ACMediaFeatures*> &obj2, const vector<float> &weights, bool inverse_features)
{
    ...
    for (int f=0; f<feature_count; f++) {
        ...
        FeaturesVector tempVector1,tempVector2;
        tempVector1=obj1[f]->getFeaturesVector();
        tempVector2=obj2[f]->getFeaturesVector();
        if (tempVector1[0]==-1000.f)
            tempVector1.set(0,0.f);
        if (tempVector2[0]==-1000.f)
            tempVector2.set(0,0.f);
...
ACNavimedAllKMeansPlugin::ACNavimedAllKMeansPlugin() : ACClusterMethodPlugin() {
    //vars herited from ACPlugin
    this->mMediaType = MEDIA_TYPE_ALL;
    this->mName = "Navimed All KMeans";
...
void ACNavimedAllKMeansPlugin::updateClusters(ACMediaBrowser* mediaBrowser,bool needsCluster){
    ...
    for ( i=0;i<currTempId1.size();i++){//scan if media have all the features.{
        ...
        for (it=featVects.begin();it!=featVects.end();it++){
            if ((*it)->getFeatureElement(0)==-1000.f){

                float randMod=(float)rand()/RAND_MAX/10.f;
                (*it)->setFeatureElement(0,randMod);
            }
        }
        ...
        for(it = 0; it < n_iterations; it++)
        {
            ...
            for(i=0; i<object_count; i++)
            {
                ...
                for(f=0; f<feature_count; f++)
                {
                    ...
                    {
                        FeaturesVector tempVector=library->getMedia(currId[i])->getPreProcFeaturesVector(f)->getFeaturesVector();
                        if (tempVector[0]==-1000.f)
                            tempVector.set(0,0.f);
                        cluster_accumulators[jmin][f].meanAdd(tempVector);
                    }
                }
