...
#include "ACVideoOpenCVReaderPlugin.h"
...
ACVideoOpenCVReaderPlugin::ACVideoOpenCVReaderPlugin() : ACMediaReaderPlugin(){
    this->mName = "Video Reader (FFmpeg)";
    this->mDescription ="Plugin for reading video files with FFmpeg";
    this->mMediaType = MEDIA_TYPE_VIDEO;
}

ACVideoOpenCVReaderPlugin::~ACVideoOpenCVReaderPlugin(){
}

ACMedia* ACVideoOpenCVReaderPlugin::mediaFactory(ACMediaType mediaType, const ACMedia* copy){
    ACMedia* media(0);
...
ACMediaData* ACVideoOpenCVReaderPlugin::mediaReader(ACMediaType mediaType){
    ACMediaData* media_data(0);
    if(mediaType&MEDIA_TYPE_VIDEO){
        media_data = new ACVideoOpenCVData();
    }
    return media_data;
}

std::map<std::string,ACMediaType> ACVideoOpenCVReaderPlugin::getSupportedExtensions(ACMediaType media_type){
