...
ACImageOsgReaderPlugin::ACImageOsgReaderPlugin() : ACMediaReaderPlugin(){
    this->mName = "Image Reader (OSG)";
    this->mDescription ="Plugin for reading image files with OpenSceneGraph";
    this->mMediaType = MEDIA_TYPE_IMAGE;
}

ACImageOsgReaderPlugin::~ACImageOsgReaderPlugin(){
}

ACMedia* ACImageOsgReaderPlugin::mediaFactory(ACMediaType mediaType, const ACMedia* copy){
    ACMedia* media(0);
