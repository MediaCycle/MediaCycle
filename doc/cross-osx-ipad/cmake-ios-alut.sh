...
# tested with ALUT v. 0.9.80

if [ "$1" = "-h" -o "$1" = "--help" -o -z "$1" ]; then
	echo "Usage: $0 {Device,Simulator} {ALUT Source Dir} [ALUT Install Dir] [iOS SDK Version = 3.2]"
	exit
fi

if ! type 'cmake' > /dev/null 2>&1; then
	echo "cmake is not found, please install cmake command which is required to build ALUT."
	exit 1;
fi
...
if [ -z "$2" ]; then
	echo "Please assign path to ALUT source directory which includes CMakeLists.txt."
	exit 1
else
	ALUT_ROOT="$2"
fi

if [ ! -f "${ALUT_ROOT}/CMakeLists.txt" ]; then
	echo "No CMakeLists.txt in ${ALUT_ROOT}, please select ALUT source directory."
	exit 1
fi

if [ -z "$3" ]; then
	INSTALL_PREFIX="`pwd`/../ALUT_${TARGET_SDK}"
else
	INSTALL_PREFIX="$3"
fi
...
echo "ALUT Root           = $ALUT_ROOT"
echo "ALUT Install Prefix = $INSTALL_PREFIX"
echo ""
