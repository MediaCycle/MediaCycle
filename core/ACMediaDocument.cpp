...
void ACMediaDocument::saveXMLSpecific(TiXmlElement* _media){
    if(this->getContainer().size() > 0){
        _media->SetAttribute("NumberOfMedias", this->getContainer().size());
        if( this->activeMedia!=0 )
            _media->SetAttribute("ActiveMediaKey", this->getActiveMediaKey());
    }
}
...
int ACMediaDocument::loadXMLSpecific(TiXmlElement* _pMediaNode){
    ...
    string pKey ="";
    pKey = _pMediaNode->Attribute("ActiveMediaKey");
    if (pKey == "")
