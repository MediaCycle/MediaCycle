...
void model3DS::loadModel(string filename, float scale, string n_diffuseTexturePath) {
    diffuseTexturePath = n_diffuseTexturePath;
    filename = ofToDataPath(filename);
	loadModel((const char *)filename.c_str(), scale);
}
		...
		switch(chunkHeader){
			...
			case CHUNK_BUMP_MAP:
                ...
                if (diffuseTexturePath.length() > 0) {
                    m_filepath = "";
                    m_tempString = diffuseTexturePath;
                }
...
void material3DS::loadTexture(std::string filename, int chunkType){
    ...
    // filename = "data/hal/hallwaydiff.jpg";
    // std::cout << "filename as given: " << filename << std::endl;
	...
	if((lowerCaseStr.find(".jpg") == std::string::npos) && (lowerCaseStr.find(".png") == std::string::npos) && (lowerCaseStr.find(".tga") == std::string::npos) && (lowerCaseStr.find(".bmp") == std::string::npos)){
		...
		// std::cout<<"[3DS] WARNING: Try to load map '"<<filename<<"'\n[3DS] WARNING: (texture must be TGA, PNG, JPG or BMP)"<<std::endl;
		return;
	}
