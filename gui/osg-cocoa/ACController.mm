...
{
	//CF uncommented
	mOscReceiver = TiOscReceiverCreate(NULL, 12346);
...
- (void) awakeFromNib
{
	...
	//CF//media_cycle->addPlugin ("/Users/dtardieu/src/Numediart/ticore-app/Applications/Numediart/MediaCycle/src/Builds/darwin-x86/plugins/visualisation/Debug/mc_visualisation.dylib");
	//CF//media_cycle->setVisualisationPlugin("Visualisation");
...
- (void)processOscMessage:(const char*)tagName
{
	...
	if(strcasecmp(tagName, "/imagecycle/1/browser/1/move/xy") == 0)
	{
		...
		//audio_cycle->getAudioBrowser()->incrementLoopNavigationLevels(loop);
		//audio_cycle->getAudioBrowser()->setSelectedObject(loop);
		//audio_cycle->getAudioBrowser()->updateClusters(true);
		media_cycle->setNeedsDisplay(true);
		
		id msg = [NSDictionary dictionaryWithObjectsAndKeys:@"movexy", @"name", [NSNumber numberWithFloat:x], @"x", [NSNumber numberWithFloat:y], @"y", nil];
	...
	else if(strcasecmp(tagName, "/imagecycle/1/browser/1/move/zoom") == 0)
	{
		...
		id msg = [NSDictionary dictionaryWithObjectsAndKeys:@"zoom", @"name", [NSNumber numberWithInt:zoom], @"val", nil];
