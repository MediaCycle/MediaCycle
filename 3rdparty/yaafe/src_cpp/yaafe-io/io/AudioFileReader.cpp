...
AudioFileReader::AudioFileReader() :
    ...
    m_start_time(-1), m_end_time(-1), m_frames_read(0), m_frames_to_read(0){
}
...
bool AudioFileReader::openFile(const std::string& filename) {
    ...
    int frame_seek = 0;
    if (m_start_time != -1)
        frame_seek = (int)std::ceil((double)m_start_time * (double)m_sfinfo.samplerate / (double)m_sfinfo.channels);

    int res = sf_seek(m_sndfile,frame_seek,SEEK_SET);
    if (res!=frame_seek)
    {
        cerr << "ERROR: cannot seek user-defined start in audio file !" << endl;
        return false;
    }
...
ParameterDescriptorList AudioFileReader::getParameterDescriptorList() const {
    ...
    p.m_identifier = "StartTime";
    p.m_description = "Time offset in seconds of the file after which the processing is started.";
    p.m_defaultValue = "-1";
    pList.push_back(p);

    p.m_identifier = "EndTime";
    p.m_description = "Time offset in seconds of the file before which the processing is ended.";
    p.m_defaultValue = "-1";
    pList.push_back(p);
...
bool AudioFileReader::init(const ParameterMap& params, const Ports<StreamInfo>& in)
{
    ...
    m_start_time = getDoubleParam("StartTime",params);
    m_end_time = getDoubleParam("EndTime",params);
    ...
    if(m_start_time !=-1 && m_end_time !=-1){
        if(m_start_time <0){
            cerr << "ERROR: wrong start time " << endl;
            return false;
        }
        if(m_end_time <=0 || m_end_time < m_start_time || m_end_time > (double)m_sfinfo.frames * (double)m_sfinfo.samplerate){
            cerr << "ERROR: wrong end time " << endl;
            return false;
        }
        m_frames_to_read = (int)std::ceil( (m_end_time-m_start_time) * (double)m_sfinfo.samplerate / (double)m_sfinfo.channels );
    }
    else
        m_frames_to_read =  (int)std::ceil((double)m_sfinfo.frames / (double)m_sfinfo.channels);
...
bool AudioFileReader::process(Ports<InputBuffer*>& inp, Ports<OutputBuffer*>& outp)
{
    ...
    if (nbRead==0)
        return false;
    m_frames_read += nbRead;

    if (m_frames_read >= m_frames_to_read)
        return false;
