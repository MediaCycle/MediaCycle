...
void write_Triads(std::vector<Triad> &ts, char * fname){
   ...
   for (int r = 0; r < nr; r++){
     ...
     << ts[r].ab+1 <<' ' << ts[r].ac+1 <<' ' << ts[r].bc+1 << " " << ts[r].ro <<  std::endl;
   }
   out.close();
...
void T_flip2( std::vector<Shx> &pts, std::vector<Triad> &triads, int *slump, int numt, int start){
  ...
  for( int t=start; t<numt; t++){
    ...
    std::cerr << "fuck up at line 12 dude!" << std::endl;
      }
...
void s_hull_del_ray2( std::vector<Shx> &pts, std::vector<Triad> &triads)
{
  ...
  //std::std::cerr << "earwig noodles " << pts[1].id << " " << pts[mid].id << " " << romin2 << std::endl;
  ...
  for( int k=3; k<nump; k++){
    ...
    if(0){
    std::cerr << "numt = " << triads.size() << std::endl;
    std::cerr << "ids = [ " ;
    for(int g=0; g<numh; g++){
      std::cerr << hull[g].id+1 << ' ' ;
    }
    std::cerr <<   hull[0].id+1 << "];" << std::endl;
    

    std::cerr << "trids = [ " ;
    for(int g=0; g<numh; g++){
      std::cerr << hull[g].trid+1 << ' ' ;
    }
    std::cerr <<   hull[0].trid+1 << "];" << std::endl;
    ...
    std::cerr <<   -(hull[0].c-hull[numh-1].c) * hull[numh-1].tr +
               (hull[0].r-hull[numh-1].r)*hull[numh-1].tc << "];" << std::endl;
...
void T_flip3( std::vector<Shx> &pts, std::vector<Triad> &triads, int *slump, int numt, int start, std::set<int> &ids){
  ...
  for( int t=start; t<numt; t++){
    ...
    std::cerr << "fuck up at line 12 dude! " << t << std::endl;
      }
    ...
    std::cerr << "fuck up at line 13 dude! " << t << std::endl;
      }
    ...
    std::cerr << "fuck up at line 14 dude! " << t << std::endl;
      }
  ...
  //  std::cerr << " triangles to resolve " << ids.size() << std::endl;
...
void T_flip4( std::vector<Shx> &pts, std::vector<Triad> &triads, int *slump, std::set<int> &ids){
  ...
  while(x != ids.end() ){
    ...
    std::cerr << "fuck up at line 15 dude! t: " << t << "  T2: " <<  T2<<  std::endl;
	//	tri.prnt();
	//t2.prnt();
    ...
    std::cerr << "fuck up at line 16 dude! " << t <<  std::endl;
      }
    ...
    std::cerr << "fuck up at line 17 dude! " << t << std::endl;
      }
	...
	if( -scubit*1000 > tri.ro )
      ...
      //std::cerr << "duff " << t << " scubit " << scubit << std::endl;
	}
  ...
  //  std::cerr << " triangles to resolve " << ids2.size() << std::endl;
  ids.clear();
