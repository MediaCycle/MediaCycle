...
arma::mat linearnerv(arma::mat desc_m)
{
    arma::mat pos;
    if(desc_m.n_rows == 0 || desc_m.n_cols == 0)
        return pos;
        ...
        if (args.hasArg ("--inputfile"))
    {
      ...
      loader.loadData (origData);*/
        ...
        DataMatrix datamatrix (desc_m.n_rows, desc_m.n_cols);
        for(int i=0;i<desc_m.n_rows; i++){
            for(int j=0;j<desc_m.n_cols; j++){
                datamatrix(i,j) = desc_m(i,j);
            }
        }
        origData = DataSet(datamatrix);

        //SOMPackExporter exporter ("./test.txt");
        //exporter.exportData(datamatrix);
    ...
    {*/
        ...
        /*iif (args.hasArg ("--nexactpercentage"))
    {
   ...
   {*/
    ...
    {*/
        ...
        if (nclusters > 0)
        {
            ...
            {
                throw std::runtime_error("LinearNeRV Error: number of rows in projected data does not match number of rows in original data!\n"); // projData.getRows(), ndata
                //exit(1);
            }
    ...
    {*/
        ...
        pos.set_size(projData.getRows(),projData.getCols());

        for(int i=0;i<projData.getRows(); i++){
            for(int j=0;j<projData.getCols(); j++){
                pos(i,j) = projData(i,j);
            }
        }
    ...
    catch (NEWMAT::BaseException/*Exception*/ & exc)
    {
