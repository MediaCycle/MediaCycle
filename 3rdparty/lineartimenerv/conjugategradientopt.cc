...
ConjugateGradientOpt::perform (DataMatrix & projData)
{
  ...
  sqsum = costFunc.getGradient (projData, gradient);

  /*
#define TEMPGRADITERS 100
  for (size_t k=0; k <TEMPGRADITERS; k++)
  {
    sqsum = costFunc.getGradient (projData, gradient);

    for (size_t i=0; i <projData.getRows(); i++)
      for (size_t j=0; j <projData.getCols(); j++)
	projData(i,j) -= gradient(i,j);

  }
  feedback << "Simple gradient step finished, cost now "
