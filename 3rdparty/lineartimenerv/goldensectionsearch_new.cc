...
bool
GoldenSectionSearch::findInitialBracket (CostFunction & costFunction,
    Matrix & pointA,
    const Matrix & negativeGradient,
    double initialStepSize)
{
  ...
  else
  {
    ...
    //      std::cerr<<"lineserach bracketing cost:"<< stepx <<"( " <<costA<<" , "<<currentMinimumCost << " , "<<costX<<" ) diff:"<<costX-currentMinimumCost<<std::endl;
...
(CostFunction & costFunction, Matrix & point,
 const Matrix & negativeGradient, double initialStepSize, double &finalCost)
{
  ...
  if (costA < costC)
  {
    ...
    {
      finalCost = costA;
      //      std::cerr<<"lineserach end cost:  " <<costA<<"\n";
      {
        for (size_t i = 0; i < point.getRows (); i++)
          for (size_t j = 0; j < point.getCols (); j++)
            point (i, j) =
  ...
  else
  {
    ...
    {
      finalCost = costC;
      //      std::cerr<<"lineserach end cost:  " <<costC<<"\n";
      for (size_t i = 0; i < point.getRows (); i++)
        for (size_t j = 0; j < point.getCols (); j++)
          point (i, j) =
