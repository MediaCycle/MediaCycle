...
//  printf("GoldenSectionSearch A1\n");
...
#ifdef DEBUGMODE  
  printf("GoldenSectionSearch Start: costA (step 0) = %e, costB (step %e) =%e\n", costA, initialStepSize, costB);
#endif
...
#ifdef DEBUGMODE  
      printf("GoldenSectionSearch : costX (step %e) = %e, currentMinimumCost %e\n", stepx, costX, currentMinimumCost);
#endif      
    }
  ...
  else
  {
    ...
    do
    {
      ...
      stepx *= INITIAL_BRACKETING_MAGNIFICATION;
#ifdef DEBUGMODE  
      printf("GoldenSectionSearch: evaluating cost function\n");
#endif      
      costX = costFunction.evaluate (x);
#ifdef DEBUGMODE  
      printf("GoldenSectionSearch : costX (step %e) = %e, currentMinimumCost %e\n", stepx, costX, currentMinimumCost);
#endif
      //printf("stepx=%f, currentMinimumCost=%f\n", stepx, currentMinimumCost);
    }
...
#ifdef DEBUGMODE  
      printf("GoldenSectionSearch : reached max iter with costX=%e, costB=%e\n", costX, costB);      
#endif      
    }
    else
    {
      costC = costX;
#ifdef DEBUGMODE  
      printf("GoldenSectionSearch : set costC = %e\n", costC);      
#endif      
    }
  }
...
#ifdef DEBUGMODE  
    printf("GoldenSectionSearch : starting position was the best found!\n");
#endif
    return false;
  }
  else
...
{
//  printf("GoldenSectionSearch 1\n");


  x = point;
//  printf("GoldenSectionSearch 2\n");
  referencePoint = point;
//  printf("GoldenSectionSearch 3\n");

  if (!findInitialBracket
      (costFunction, point, negativeGradient, initialStepSize))
    return 0.0;
//  printf("GoldenSectionSearch 4\n");
...
#ifdef DEBUGMODE
  printf("GoldenSectionSearch : reached final cost %e at step %e\n", finalCost, nextInitialStep);
#endif
