...
void FFmpegDecoderVideo::open(AVStream * const stream)
{
    ...
    m_buffer_rgba[0].resize(avpicture_get_size(PIX_FMT_RGB24, width(), height()));//CF PIX_FMT_RGB32?
    m_buffer_rgba[1].resize(m_buffer_rgba[0].size());

    // Assign appropriate parts of the buffer to image planes in m_frame_rgba
    avpicture_fill((AVPicture *) (m_frame_rgba).get(), &(m_buffer_rgba[0])[0], PIX_FMT_RGB24, width(), height());//CF PIX_FMT_RGB32?
...
void FFmpegDecoderVideo::publishFrame(const double delay, bool audio_disabled)
{
    ...
    // Assign appropriate parts of the buffer to image planes in m_frame_rgba
    avpicture_fill((AVPicture *) (m_frame_rgba).get(), &(m_buffer_rgba[m_writeBuffer])[0], PIX_FMT_RGB24, width(), height());//CF PIX_FMT_RGB32?
    ...
    if (m_context->pix_fmt == PIX_FMT_YUVA420P)
        ...
        convert(dst, PIX_FMT_RGB24, src, m_context->pix_fmt, width(), height());//CF PIX_FMT_RGB32?
