...
#endif
{
    ...
    for (i=0;i<OPENAL_NUM_BUFFERS;i++) {
        resample_ratios[i] = active_resample_ratio;//1.0;
    }

#ifdef USE_OPENALEXT
    bool init = initEffect(&reverb,&effect,&slot);
#endif
}
...
void ACAudioFeedback::setResampleRatio(float resample){
    if(resample < 0) resample = 0;
    if(resample > 10) resample = 10;
    active_resample_ratio = resample;
    for (int i=0;i<OPENAL_NUM_BUFFERS;i++) {
        resample_ratios[i]=resample;
    }
}
...
int ACAudioFeedback::createSourceSynchro()
{
    ...
    /* Connect the source to the effect slot. This tells the source to use the
             ...
             */
    alSource3i(loop_source, AL_AUXILIARY_SEND_FILTER, slot, 0, AL_FILTER_NULL);
    if(alGetError()!=AL_NO_ERROR) std::cerr << "Failed to setup sound source" << std::endl;
#endif
...
int ACAudioFeedback::createSourceWithPosition(int loop_id, float x, float y, float z)
{
    ...
    if (dataf) {
        ...
        resample_ratio = 1; //CF MashtaCycle hack
...
int ACAudioFeedback::setSourceScaleMode(int loop_id, ACAudioEngineScaleMode mode)
{
#ifdef USE_OPENAL
     int loop_slot;
    ALuint	loop_source;
    ...
    loop_scale_mode[loop_slot] = mode;
    return 0;
#endif
}

int ACAudioFeedback::setSourceSynchroMode(int loop_id, ACAudioEngineSynchroMode mode)
{
#ifdef USE_OPENAL
    int loop_slot;
    ALuint	loop_source;
    ...
    loop_synchro_mode[loop_slot] = mode;
    return 0;
#endif
}

int ACAudioFeedback::setSourceBPM(int loop_id, float bpm)
{
#ifdef USE_OPENAL
    if(bpm <0)
        bpm = 0;
    ...
    if(loop_synchro_mode[loop_slot]!=ACAudioEngineSynchroModeAutoBeat)
        loop_synchro_mode[loop_slot] =ACAudioEngineSynchroModeAutoBeat;
    if(loop_scale_mode[loop_slot]!=ACAudioEngineScaleModeResample)
        loop_scale_mode[loop_slot]=ACAudioEngineScaleModeResample;
    use_bpm[loop_slot] = bpm;
    return 0;
#endif
}

int ACAudioFeedback::setSourceResampleRatio(int loop_id, float resample){
#ifdef USE_OPENAL
    if(resample <0)
        resample = 0;
    if(resample > 10)
    ...
    resample_ratios[loop_slot] = resample;
    return 0;
#endif
    return 1;
}
