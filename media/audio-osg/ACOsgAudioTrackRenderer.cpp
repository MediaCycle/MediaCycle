...
void ACOsgAudioTrackRenderer::selectionCursorGeode() {
 ...
 (*vertices)[1] = Vec3(track_left_x + 0, ((summary_center_y + summary_height)-xstep) * zoom_y, zpos+0.00005);*/
    (*vertices)[0] = Vec3((selection_end_pos_x+selection_begin_pos_x)/2.0f, ((summary_center_y - summary_height)+xstep) * zoom_y, zpos);
    (*vertices)[1] = Vec3((selection_end_pos_x+selection_begin_pos_x)/2.0f, ((summary_center_y + summary_height)-xstep) * zoom_y, zpos);
...
void ACOsgAudioTrackRenderer::selectionZoneGeode() {
    ...
    //border vertices
    vertices = new Vec3Array(5);
    (*vertices)[0] = Vec3(selection_begin_pos_x, (summary_center_y - summary_height) * zoom_y, zpos);
    (*vertices)[1] = Vec3(selection_end_pos_x, ((summary_center_y - summary_height)+xstep) * zoom_y, zpos);
    (*vertices)[2] = Vec3(selection_end_pos_x, ((summary_center_y + summary_height)-xstep) * zoom_y, zpos);
    (*vertices)[3] = Vec3(selection_begin_pos_x, (summary_center_y + summary_height) * zoom_y, zpos);
    (*vertices)[4] = Vec3(selection_begin_pos_x, (summary_center_y - summary_height) * zoom_y, zpos);
    selection_zone_border_geometry->setVertexArray(vertices);

    vertices = new Vec3Array(4);
    (*vertices)[0] = Vec3(selection_begin_pos_x+selection_sensing_width, (summary_center_y - summary_height) * zoom_y, zpos);
    (*vertices)[1] = Vec3(selection_end_pos_x-selection_sensing_width, (summary_center_y - summary_height) * zoom_y, zpos);
    (*vertices)[2] = Vec3(selection_end_pos_x-selection_sensing_width, (summary_center_y + summary_height) * zoom_y, zpos);
    (*vertices)[3] = Vec3(selection_begin_pos_x+selection_sensing_width, (summary_center_y + summary_height) * zoom_y, zpos);
...
void ACOsgAudioTrackRenderer::selectionBeginGeode() {
    ...
    vertices = new Vec3Array(4);
    (*vertices)[0] = Vec3(selection_begin_pos_x, (summary_center_y - summary_height) * zoom_y, zpos);
    (*vertices)[1] = Vec3(selection_begin_pos_x+selection_sensing_width, (summary_center_y - summary_height) * zoom_y, zpos);
    (*vertices)[2] = Vec3(selection_begin_pos_x+selection_sensing_width, (summary_center_y + summary_height) * zoom_y, zpos);
    (*vertices)[3] = Vec3(selection_begin_pos_x, (summary_center_y + summary_height) * zoom_y, zpos);
...
void ACOsgAudioTrackRenderer::selectionEndGeode() {
    ...
    vertices = new Vec3Array(4);
    (*vertices)[0] = Vec3(selection_end_pos_x-selection_sensing_width, (summary_center_y - summary_height) * zoom_y, zpos);
    (*vertices)[1] = Vec3(selection_end_pos_x, (summary_center_y - summary_height) * zoom_y, zpos);
    (*vertices)[2] = Vec3(selection_end_pos_x, (summary_center_y + summary_height) * zoom_y, zpos);
    (*vertices)[3] = Vec3(selection_end_pos_x-selection_sensing_width, (summary_center_y + summary_height) * zoom_y, zpos);
...
void ACOsgAudioTrackRenderer::playbackWaveformGeode() {
    ...
    if (samples && screen_width !=0){
        ...
        float n_frames = (float)(((ACAudio*) media)->getNFrames());
        float sample_rate = (float)(((ACAudio*) media)->getSampleRate());
        float hop = n_frames * (selection_end_pos_x-selection_begin_pos_x)/xspan / sample_rate / (float)(width);
        n_playback_samples = (int)(n_frames * (selection_end_pos_x-selection_begin_pos_x)/xspan);
        n_samples_hop = hop * sample_rate;
        ...
        if (n_playback_samples >= 10*screen_width){
            thumbnail = new float[2 * width];
            int k = (int)( n_frames *(selection_begin_pos_x+xspan/2.0f)/xspan);
            for (int i=0; i< width; i++) {
                if ((k < 0)||(k>n_frames)){
                    thumbnail[i] = 0.0f;
            ...
            width = (int)(n_frames * (selection_end_pos_x-selection_begin_pos_x)/xspan);
            thumbnail = new float[width];
            int k = (int)( n_frames *(selection_begin_pos_x+xspan/2.0f)/xspan);
            for (int i=0; i< width; i++) {
                if ((k+i<0)||(k+i>=width))
  ...
  std::cout << "  playback frames: " << (((ACAudio*) media)->getNFrames()) * (selection_end_pos_x-selection_begin_pos_x)/xspan << std::endl;
