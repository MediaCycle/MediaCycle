...
//	void computeImageHistogram(int w, int h); // XS TODO port 2.*
	...
	void computeNumberOfFaces(const std::string& cascadeFile, const float& scale=1.3, const float& searchScaleFactor=1.2, const int& minNeighbors=3,  const int& flags=cv::CASCADE_SCALE_IMAGE, const cv::Size& minFeatureSize=cv::Size(20, 20));
