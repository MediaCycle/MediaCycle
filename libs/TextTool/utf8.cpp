...
//this function was not taken from gnome
size_t mc_utf8charlen(const unsigned char c)
{
	int mask = 0;
	int len=0;
...
std::string mc_wcstoutf8string(const wchar_t* str, size_t strlen){
	size_t i = 0;
	std::string result;
	char p[6];
	
	while (i < strlen && str[i] != 0){
		result.append(p, mc_wctoutf8(p,str[i++]));
	}
