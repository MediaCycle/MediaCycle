...
{
    if(!view){
        std::cerr << "ACInputControlsDialogQt::on_pushButtonAddTag_clicked: requires a view that isn't set, can't add tag." << std::endl;
        return;
    }
    ...
    if(dialog->exec()){
        ...
        pAction->setParent(/*dynamic_cast<QObject*>(*/this->parent/*)*/);
