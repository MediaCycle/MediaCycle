/**
 * @brief Main MediaCycle window wrapping an abstract view of a MediaCycle browser
 * @author Christian Frisson, Xavier Siebert
 * @date 16/11/2010
...
ACMultiMediaCycleQt::ACMultiMediaCycleQt(ACAbstractViewQt* _view, QWidget *parent)
    : QMainWindow(parent),ACEventListener(),features_known(false),
      aboutDialog(0),controlsDialog(0),view(_view),
      viewDockWidget(0),viewDockLayout(0),progressBar(0),metadataWindow(0),
      ...
      userProfileWindow(0),segmentationDialog(0),librarySaveDialog(0)
{
    ...
    viewDockWidget = new QWidget; // it seems this intermediary widget is required to set a layout to a dock widget
    viewDockLayout = new QVBoxLayout;
    viewDockLayout->setSpacing(0); // no blank space between the progress bar and the osg view
    viewDockLayout->setContentsMargins(0,0,0,0);// no unnecessary corners in the osg view dock widget (this supersedes the OS theme defaults)
    viewDockWidget->setLayout(viewDockLayout);

    //view = new ACMultiMediaCycleQt();
    //CF connect( view, SIGNAL( importDirectoriesThreaded(std::vector<std::string>,bool ) ),
    //   this, SLOT(importDirectoriesThreaded(std::vector<std::string>,bool) ) );
    viewWidget = dynamic_cast<QWidget*>(view);
    viewWidget->setSizePolicy ( QSizePolicy::MinimumExpanding, QSizePolicy::MinimumExpanding );
    viewDockLayout->addWidget(viewWidget);

    progressBar = new ACQProgressBar(); //QProgressBar();
    viewDockLayout->addWidget(progressBar);
    //this->setCentralWidget(viewDockWidget);

    QWidget* container = new QWidget;
    container->setLayout(viewDockLayout);
...
ACMultiMediaCycleQt::~ACMultiMediaCycleQt(){
    this->clean();
    ...
    if (viewDockLayout) delete viewDockLayout;
    if (viewDockWidget) delete viewDockWidget;
...
void ACMultiMediaCycleQt::mediaImported(int n,int nTot,int mId){
    std::cout << "ACMultiMediaCycleQt::mediaImported media id " << mId << " ("<< n << "/" << nTot << ")" << std::endl;
    std::string send = "";
    if (n==0) { //  && n!=nTot
...
void ACMultiMediaCycleQt::pluginLoaded(std::string plugin_name){
    if(!media_cycle){
        std::cout << "ACMultiMediaCycleQt::pluginLoaded: mediacycle not set" << std::endl;
        return;
    }
    if(!dockWidgetsManager){
        std::cout << "ACMultiMediaCycleQt::pluginLoaded: dock widgets manager not set" << std::endl;
        return;
    }
    ...
    if(renderer_plugin && this->view){
        renderer_plugin->setBrowser(this->view->getBrowser());
        renderer_plugin->setTimeline(this->view->getTimeline());
    }
    QObject* qobject = dynamic_cast<QObject*>(plugin);
    if(!qobject){
        //std::cerr << "ACMultiMediaCycleQt::pluginLoaded: " << plugin_name << " not an ACPluginQt" << std::endl;
        return;
    }
    //std::cout << "ACMultiMediaCycleQt::pluginLoaded: " << plugin_name << " is a QObject" << std::endl;
    ACPluginQt* qt_plugin = qobject_cast<ACPluginQt*>(qobject);
    if(!qt_plugin){
        //std::cerr << "ACMultiMediaCycleQt::pluginLoaded: " << plugin_name << " not an ACPluginQt" << std::endl;
        return;
    }
    std::cout << "ACMultiMediaCycleQt::pluginLoaded: " << plugin_name << " is an ACPluginQt" << std::endl;
    /*std::string dockName = plugin->dockName();
    if(dockName!=""){
        std::cout << "ACMultiMediaCycleQt::pluginLoaded: adding dock " << dockName << " for plugin " << plugin_name << std::endl;
...
void ACMultiMediaCycleQt::configureSettings(){
...
void ACMultiMediaCycleQt::createMediaCycle(ACMediaType _media_type, ACBrowserMode _browser_mode){
...
void ACMultiMediaCycleQt::destroyMediaCycle(){
    delete media_cycle;
}


// XS in theory one could select multiple XML files and concatenate them (not tested yet)
void ACMultiMediaCycleQt::on_actionLoad_Library_triggered(bool checked){
    this->readXMLConfig();
}

bool ACMultiMediaCycleQt::readXMLConfig(string _filename){
    if (_filename==""){
        QString fileName;
    ...
    try {
        ...
        if( ACMediaFactory::getInstance().getExtensionsFromMediaType(this->media_type).size() == 0){
            std::cerr << "ACMultiMediaCycleQt::readXMLConfig: old XML file, trying to match one of the available default configs" << std::endl;
            needs_saving=true;
        }
        if(!media_cycle->getCurrentConfig()){ // happens when loading an XML file from a newly-launched MultiMediaCycle
            ...
            for(std::vector<std::string>::iterator defaultConfig = defaultConfigs.begin(); defaultConfig != defaultConfigs.end();++defaultConfig){
                ...
                if(config.contains(mediaType) && config.contains("cycle")){
                    std::cout << "ACMultiMediaCycleQt::readXMLConfig: loading config " << *defaultConfig << std::endl;
...
void ACMultiMediaCycleQt::on_actionSave_Library_triggered(bool checked){
...
void ACMultiMediaCycleQt::writeXMLConfig(string _filename){
...
void ACMultiMediaCycleQt::on_actionLoad_Media_Directory_triggered(bool checked){
...
void ACMultiMediaCycleQt::on_actionLoad_Media_Files_triggered(bool checked){
...
void ACMultiMediaCycleQt::importDirectoriesThreaded(vector<string> directories,bool askToSegment) {
...
bool ACMultiMediaCycleQt::doSegments(){
    bool do_segments = false;
...
void ACMultiMediaCycleQt::on_actionEdit_Config_File_triggered(bool checked){
...
void ACMultiMediaCycleQt::on_actionEdit_Input_Controls_triggered(bool checked){
    ...
    controlsDialog->setView(this->view);
...
bool ACMultiMediaCycleQt::addControlDock(std::string dock_type){
    if(!dockWidgetsManager)
        return false;
...
bool ACMultiMediaCycleQt::addControlDock(ACPluginType plugin_type){
    if(!dockWidgetsManager)
        return false;
...
bool ACMultiMediaCycleQt::addControlDock(ACAbstractDockWidgetQt* dock)
{
    if(dock){
...
void ACMultiMediaCycleQt::on_actionToggle_Controls_triggered(bool controlsToEnable){
    if(dockWidgetsManager)
        dockWidgetsManager->updateDocksVisibility(controlsToEnable);
}

bool ACMultiMediaCycleQt::addAboutDialog(ACAbstractAboutDialogQt* dialog)
{
    if (dialog == 0){
...
bool ACMultiMediaCycleQt::addAboutDialog(std::string about_type)
{
    return this->addAboutDialog(aboutDialogFactory->createAboutDialog(this,about_type));
}

void ACMultiMediaCycleQt::on_actionHelpAbout_triggered(bool checked) {
    if (aboutDialog==0)
...
void ACMultiMediaCycleQt::on_actionEdit_Library_Metadata_triggered(bool checked) {
    if (metadataWindow)
        metadataWindow->show();
}

void ACMultiMediaCycleQt::on_actionEdit_Profile_triggered(bool checked) {
    if (userProfileWindow)
        userProfileWindow->show();
}

void ACMultiMediaCycleQt::on_actionFullscreen_triggered(bool checked) {
    if (viewWidget->isFullScreen() || this->isFullScreen()){
        std::cout << "Not fullscreen" << std::endl;
        this->showNormal();
        viewWidget->showNormal();
...
void ACMultiMediaCycleQt::dragEnterEvent(QDragEnterEvent *event)
{
    std::cout <<"<drop content>" << std::endl;
    event->acceptProposedAction();
}

void ACMultiMediaCycleQt::dragMoveEvent(QDragMoveEvent *event)
{
    event->acceptProposedAction();
}

void ACMultiMediaCycleQt::dropEvent(QDropEvent *event)
{
...
void ACMultiMediaCycleQt::dragLeaveEvent(QDragLeaveEvent *event)
{
    event->accept();
}


void ACMultiMediaCycleQt::updateLibrary(){
    if (! hasMediaCycle()) return;
...
bool ACMultiMediaCycleQt::addPluginLibrary(string _library){
    bool ok = false;
...
bool ACMultiMediaCycleQt::removePluginLibrary(string _library){
    bool ok = false;
    if (! hasMediaCycle()) return false;
...
ACPluginLibrary* ACMultiMediaCycleQt::getPluginLibrary(string _library) {
    if (! hasMediaCycle()) return 0;
    return this->getMediaCycle()->getPluginLibrary(_library);
}

bool ACMultiMediaCycleQt::removePluginFromLibrary(std::string _plugin_name, std::string _library_path){
    if (! hasMediaCycle()) return 0;
...
std::string ACMultiMediaCycleQt::rstrip(const std::string& s){
    std::string::size_type p = s.find_last_not_of(" \n\r\t");
    return std::string(s, 0, p+1);
}

int ACMultiMediaCycleQt::tryLoadFeaturePluginFromBaseName(std::string basename)
{
    if (!media_cycle)
        return -1;
...
bool ACMultiMediaCycleQt::addDefaultConfig(ACAbstractDefaultConfig* _config)
{
    if(!media_cycle){
        std::cerr << "ACMultiMediaCycleQt::addDefaultConfig: requires a MediaCycle instance" << std::endl;
        return false;
    }
    if(!(media_cycle->addDefaultConfig(_config))){
        std::cerr << "ACMultiMediaCycleQt::addDefaultConfig: wrong config" << std::endl;
        return false;
    }
...
bool ACMultiMediaCycleQt::loadDefaultConfig(ACAbstractDefaultConfig* _config)
{
    if(media_cycle->getCurrentConfig()){
    ...
    if(!(this->addDefaultConfig(_config)))
        return false;
    ACAbstractDefaultConfigQt* _qt_config = dynamic_cast<ACAbstractDefaultConfigQt*>(_config);
...
void ACMultiMediaCycleQt::changeMediaConfig(QString media){
    ...
    if(!media_cycle){
        std::cerr << "ACMultiMediaCycleQt::changeMediaConfig: requires a MediaCycle instance" << std::endl;
        return;
    }

    ACAbstractDefaultConfig* config = media_cycle->getDefaultConfig(mt);
    if(!config){
        std::cerr << "ACMultiMediaCycleQt::changeMediaConfig: no config named '" << mt << "'" << std::endl;
        return;
    }
...
void ACMultiMediaCycleQt::changeActiveMediaType(QString name){
#ifdef SUPPORT_MULTIMEDIA
    string nameStr=string(name.toStdString());
...
void ACMultiMediaCycleQt::on_actionClean_triggered(bool checked) {
    this->clean();
}

void ACMultiMediaCycleQt::clean(bool _updategl){
...
void ACMultiMediaCycleQt::showError(std::string s){
    int warn_button;
...
void ACMultiMediaCycleQt::showError(const exception& e) {
    this->showError(e.what());
}

bool ACMultiMediaCycleQt::hasMediaCycle(){
...
void ACMultiMediaCycleQt::closeEvent(QCloseEvent *event) {
...
bool ACMultiMediaCycleQt::readQSettings() {
...
bool ACMultiMediaCycleQt::writeQSettings() {
...
void ACMultiMediaCycleQt::clearQSettings() {
    QSettings settings("numediart", "MediaCycle");
    settings.clear();
}

void ACMultiMediaCycleQt::setDefaultQSettings() {
...
void ACMultiMediaCycleQt::changeMediaType(ACMediaType _media_type){
...
void ACMultiMediaCycleQt::useSegmentationByDefault(bool _status)
{
    use_segmentation_default = _status;
}

bool ACMultiMediaCycleQt::switchSegmentation(bool _status)
{
...
void ACMultiMediaCycleQt::switchFeatureExtraction(bool _status)
{
    use_feature_extraction = _status;
}

void ACMultiMediaCycleQt::switchPluginVisualizations(bool _status)
{
    use_visualization_plugins = _status;
}

void ACMultiMediaCycleQt::changeSetting(ACSettingType _setting)
{
    if(this->view)
