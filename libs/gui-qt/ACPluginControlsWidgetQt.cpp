        ...
        if(plugin_name.toStdString() == current_plugin_name && plugin->getParametersCount()>0){
            ...
            for(std::vector<std::string>::iterator numParamName=numParamNames.begin();numParamName!=numParamNames.end();numParamName++){
                ...
                if(min==0 && max==1 && step==1){
                    ...
                    if(tabWidget && word_count[parameter_word[*numParamName]] > 1){
                        int w = 0;
                        dynamic_cast<QGridLayout*>(tabWidget->widget(tab_index[parameter_word[*numParamName]])->layout())->addWidget(label,widget_count[parameter_word[*numParamName]],w++);
                        dynamic_cast<QGridLayout*>(tabWidget->widget(tab_index[parameter_word[*numParamName]])->layout())->addWidget(slider,widget_count[parameter_word[*numParamName]],w++);
                        dynamic_cast<QGridLayout*>(tabWidget->widget(tab_index[parameter_word[*numParamName]])->layout())->addWidget(spinbox,widget_count[parameter_word[*numParamName]],w++);
                        widget_count[parameter_word[*numParamName]] += 1;
                    }
                    else{
                        int w = 0;
                        layout->addWidget( label,paramIdx,w++);
                        layout->addWidget( slider,paramIdx,w++);
                        layout->addWidget( spinbox,paramIdx++,w++);
                        spinbox->adjustSize();
                    ...
                    connect(spinbox,SIGNAL(valueChanged(double)),numberParameter,SLOT(updateNumberParameter(double)));
                    connect(numberParameter,SIGNAL(numberParameterChanged(double)),spinbox,SLOT(setValue(double)));
                }
            }
