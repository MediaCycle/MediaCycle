...
PCA_constrained_t_Sne::PCA_constrained_t_Sne(){
    kk=30;
    tol=1e-4;
    max_iter = 1500;
}

bool PCA_constrained_t_Sne::setDistanceMatrix(arma::mat D){
...
bool PCA_constrained_t_Sne::d2p(arma::mat Dist){
    ...
    double logK=log(kk);
    for (uword i=0;i<N;i++){
        //cout<<"PCA_constrained_t_Sne::d2p"<<i<<endl;
...
void PCA_constrained_t_Sne::Hbeta(double &H,rowvec &thisP,rowvec Dist,int ind,double beta){
...
bool PCA_constrained_t_Sne::setFeatureMatrix(arma::mat X){//TODO Verify if it's necessay an ACP or not
    //cout<<"X:"<<X<<endl;
    X=X-min(min(X));
    X=X/max(max(X));
    mat coeff;
    mat M;
    princomp(coeff, M, X);
    this->X.resize(M.n_rows, 2);
    this->X.col(0)=M.col(0);
    this->X.col(1)=M.col(1);
    //cout<<"pca:"<<M<<endl;
...
/*bool PCA_constrained_t_Sne::setXInit(arma::colvec X){
    if (P.n_elem>0){
        if (X.n_rows!=P.n_cols)
            return false;
    }
//    this->X=X;
    return true;
...
arma::mat PCA_constrained_t_Sne::compute(int ndim,float lambda,arma::mat yInit){
    bool initFlag=true;
...
//        ydata=yInit;
    ...
    for (int iter=0;iter<max_iter;iter++){
        ...
        if (iter%10==0){
        cout<<"ygrad"<<mean(mean(abs(y_grads),0));
        cout<<"pca diff:"<<mean(mean(abs(this->X%(ones(n,1)*(stddev(ydata)/stddev(this->X)))-ydata)));
        }
        //y_grads=(lambda)*y_grads-(1-lambda)*0.00001*(this->X%(ones(n,1)*(stddev(ydata)/stddev(this->X)))-ydata);
        y_grads=(lambda)*y_grads-(1-lambda)*pow((5*this->X-ydata),3)/10000;
