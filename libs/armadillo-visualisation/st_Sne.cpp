...
bool st_Sne::setDistanceMatrix(arma::mat D,arma::umat label){
    ...
    for (uword i=0;i<label.n_rows;i++)
        for (uword j=0;j<label.n_rows;j++)
        {
            if (label(i,1)!=label(j,1))
                D(label(i,0),label(j,0))*=(slope);
            else
                D(label(i,0),label(j,0))/=(slope);
        }
...
*/
bool st_Sne::setFeatureMatrix(arma::mat X,arma::umat label){//TODO Verify if it's necessay an ACP or not
    //cout<<"X:"<<X<<endl;
    X=X-min(min(X));
    X=X/max(max(X));
    mat coeff;
    ...
    for (uword i=0;i<label.n_rows;i++)
        for (uword j=0;j<label.n_rows;j++)
            if (label(i,1)!=label(j,1))
                D(label(i,0),label(j,0))*=(slope*slope);
    return d2p(D);
}
/*
