...
constrained_t_Sne::constrained_t_Sne(){
    kk=30;
    tol=1e-4;
    max_iter = 1500;
}

bool constrained_t_Sne::setDistanceMatrix(arma::mat D){
...
bool constrained_t_Sne::d2p(arma::mat Dist){
    ...
    double logK=log(kk);
    for (uword i=0;i<N;i++){
        //cout<<"constrained_t_Sne::d2p"<<i<<endl;
...
void constrained_t_Sne::Hbeta(double &H,rowvec &thisP,rowvec Dist,int ind,double beta){
...
bool constrained_t_Sne::setFeatureMatrix(arma::mat X){//TODO Verify if it's necessay an ACP or not
    //cout<<"X:"<<X<<endl;
    X=X-min(min(X));
    X=X/max(max(X));
    mat coeff;
...
bool constrained_t_Sne::setXInit(arma::colvec X){
    if (P.n_elem>0){
...
arma::mat constrained_t_Sne::compute(int ndim,arma::mat yInit){
    bool initFlag=true;
