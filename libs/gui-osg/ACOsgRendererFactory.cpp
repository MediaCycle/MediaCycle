...
void ACOsgRendererFactory::pluginLoaded(std::string name){
    ...
    ACOsgRendererPlugin* plugin = dynamic_cast<ACOsgRendererPlugin*>( media_cycle->getPluginManager()->getPlugin(name) );
    if(!plugin){
    ...
    if (renderer_name != checked_renderers.end()){
        ...
        plugin->setBrowserRenderer(this->browser);
        plugin->setTimelineRenderer(this->timeline);

        std::vector<ACMediaType> media_types = plugin->getSupportedMediaTypes();
