			...
			if (view){
				...
				if(selection){
					...
					if(selecting_summary_waveform){
						...
						if (mediaID > -1){
							if (media_cycle->getLibrary()->getMedia(mediaID)->getType() == MEDIA_TYPE_AUDIO){
                                                                std::cout << "Skipping zone to frame: " << (int)((pos+1.0f)*((ACAudio*) media_cycle->getLibrary()->getMedia(mediaID))->getNFrames()) << std::endl;
								#if defined (SUPPORT_AUDIO)
								if(audio_engine){
						...
						// Visual feedback
						renderer->getTrack(selection->getRefId())->moveSelection(xspan*pos/2.0f);
						#ifdef DEBUG_HANDLE
					...
					// Skipping/Moving the current frame to the mouse position
						...
						if (mediaID > -1){
							if (media_cycle->getLibrary()->getMedia(mediaID)->getType() == MEDIA_TYPE_AUDIO){
                                                                std::cout << "Skipping current frame to frame: " << (int)((pos+1.0f)*((ACAudio*) media_cycle->getLibrary()->getMedia(mediaID))->getNFrames()) << std::endl;
								#if defined (SUPPORT_AUDIO)
								if(audio_engine){
									audio_engine->setLoopSynchroMode(mediaID, ACAudioEngineSynchroModeManual);
									audio_engine->setLoopScaleMode(mediaID, ACAudioEngineScaleModeResample);
					...
					// Skipping/Moving to the segment under the mouse
						...
						renderer->getTrack(selection->getRefId())->moveSelection(xspan*pos/2.0f); //should be adapted to the segment begin time once segmentation works that way
						#ifdef DEBUG_HANDLE
		...
		case(osgGA::GUIEventAdapter::DRAG):
		{
			...
			if (selection){
				...
				if (selecting_zone_begin){
					renderer->getTrack(selection->getRefId())->resizeSelectionFromBegin(xspan*pos/2.0f);
					#ifdef DEBUG_HANDLE
				...
				//Resizing the selection zone by moving its right boundary
					...
					renderer->getTrack(selection->getRefId())->resizeSelectionFromEnd(xspan*pos/2.0f);
					#ifdef DEBUG_HANDLE
					...
					//if ( (selection_begin+xspan*(pos-pushed_x)/2.0f > -xspan/2.0f) && (selection_end+xspan*(pos-pushed_x)/2.0f < xspan/2.0f)){//selection can't slide out the screen
						...
						if (mediaID > -1){
							if (media_cycle->getLibrary()->getMedia(mediaID)->getType() == MEDIA_TYPE_AUDIO){
								//std::cout << "Skipping to frame: " << (int)((pos+1.0f)*((ACAudio*) media_cycle->getLibrary()->getMedia(mediaID))->getNFrames()) << std::endl;
								#ifdef DEBUG_HANDLE
								std::cout << "(DRAG) Moving the selection zone (track " << selection->getRefId() <<") to audio pos " << 100.0f*(pos+1.0f)/2.0f << std::endl;
								#endif
								#if defined (SUPPORT_AUDIO)
								if(audio_engine){
									audio_engine->setLoopSynchroMode(mediaID, ACAudioEngineSynchroModeManual);
						...
						// Visual feedback
						//renderer->getTrack(selection->getRefId())->moveSelection(xspan*pos/2.0f);
						renderer->getTrack(selection->getRefId())->moveSelection(xspan*(pos+cursor_pushed_dist)/2.0f);
					//}	
				}
					...
					//{
						switch(ea.getEventType())
						{
							case(osgGA::GUIEventAdapter::PUSH):
							{
								...
								cursor_pushed_dist = renderer->getTrack(selection->getRefId())->getSelectionPos()/(xspan/2.0f) - ea.getXnormalized();
