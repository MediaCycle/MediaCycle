...
std::vector<ACMediaTimedFeature*> computeFeatures(float* data, vector<string> descList, int samplerate, int nchannels, long length, int mfccNbChannels, int mfccNb, int windowSize, bool extendSoundLimits, int minOctave, int maxOctave) {
    ...
    std::cout << "ACAudioFeatures::computeFeatures resampling";
    //with bufSize " << bufSize
    std::cout << " @ " << samplerate << " Hz and " << nchannels << " channel(s)" << std::endl;
    std::cout << "ACAudioFeatures::computeFeatures computing features with hopSize " << hopSize << " @ " << sr_hz << " Hz and 1 channel, windowSize " << windowSize << ", fftSize " << fftSize  << std::endl;
    ...
    //signal_v.save("signal_v-old.txt", arma_ascii);
    ...
    for (long i=0; i <= signal_v.n_elem-windowSize; i = i+hopSize){
        ...
        /*if(i<10*hopSize){
            ...
            file << "frameFFTabs2-old-" << i << ".txt";
            frameFFTabs2_v.save(std::string(file.str()), arma_ascii);
        }*/
        for (int iDesc = 0; iDesc < descList.size(); iDesc++){
            switch (descMap[descList[iDesc]]) {
            case SPECTRAL_CENTROID:
                sc_v(index) = spectralCentroid(frameFFTabs2_v)*b2f; // input size: fftSize/2,1
                break;
            case SPECTRAL_SPREAD:
                ss_v(index) = spectralSpread(frameFFTabs2_v)*b2f; // input size: fftSize/2,1
                break;
            case SPECTRAL_FLATNESS:
                sfm_m.row(index) = spectralFlatness(frameFFTabs2_v, fftSize, sr_hz); // input size: fftSize/2,1
                break;
            case SPECTRAL_VARIATION:
                sv_v(index) = spectralVariation(frameFFTabs2_v, prevFrameFFTabs_v); // input size: fftSize/2,1 , fftSize/2,1?
                break;
            case SPECTRAL_FLUX:
                sf_v(index) = spectralFlux(frameFFTabs2_v, prevFrameFFTabs_v); // input size: fftSize/2,1 , fftSize/2,1?
                break;
            case ZERO_CROSSING_RATE:
                zcr_v(index) = zcr(frame_v, sr_hz, windowSize); // input size: windowSize
                break;
            case SPECTRAL_DECREASE:
                sd_v(index) = spectralDecrease(frameFFTabs2_v); // input size: fftSize/2,1
                break;
            case ENERGY:
                ener_v(index) = energyRMS(frame_v);// input size: windowSize
                break;
            case LOUDNESS:
                loud_v(index) = loudness(frameFFTabs2_v, melfilter_m); // input size: fftSize/2,1
                break;
            case MFCC:
                mfcc_m.row(index) = mfcc(frameFFTabs2_v, melfilter_m, mfccNb); // input size: fftSize/2,1
                break;
            case CHROMA:
                chroma_m.row(index)= trans(trans(chromafilter_m)*(frameFFTabs2_v)); // input size: ? * fftSize/2,1
                break;
                /*case CHIRP:
                     GDspectrogram.row(index)=chirpGroupDelay(frameW_v,fftSize, windowSize, r_chirpGD); // input size: windowsSize,1?
                     break;
                     case PITCH: //!!! HNR requires that too!
                     frameW_h = frame_v%window_hanning;	// % : element-wise multiplication
                     inv=GetLPCresidual(frameW_h,orderLPC); // input size: windowsSize,1?
...
rowvec spectralFlatness(colvec x_v, int fftSize, int sr_hz){
    ...
    float f2b = (float)fftSize/(float)sr_hz;
    urowvec band_bin_v(5);
    band_bin_v = conv_to<urowvec>::from(band_hz_v * f2b);

    //colvec fftBand_v;//(band_bin_v(4));
    for (int iBand = 0; iBand < band_bin_v.n_elem-1; iBand++){
        //fftBand_v = x_v.rows(band_bin_v(iBand), band_bin_v(iBand+1)-1);
        //double* aux_mem = new double[band_bin_v(iBand+1)-band_bin_v(iBand)];
        //colvec fftBand_v(aux_mem,band_bin_v(iBand+1)-band_bin_v(iBand));
        //fftBand_v = x_v.rows(band_bin_v(iBand), band_bin_v(iBand+1)-1);//CF
        //if (sum(fftBand_v) == 0){
        if (sum(x_v.rows(band_bin_v(iBand), band_bin_v(iBand+1)-1)) == 0){
            sfm_v(iBand) = 1;
        }
        else{
            //sfm_v(iBand) = as_scalar(exp(sum(log(fftBand_v))/fftBand_v.n_elem)) / as_scalar(mean(fftBand_v));
            sfm_v(iBand) = as_scalar(exp(sum(log( x_v.rows(band_bin_v(iBand), band_bin_v(iBand+1)-1) ))/( band_bin_v(iBand+1)-band_bin_v(iBand) ))) / as_scalar(mean( x_v.rows(band_bin_v(iBand), band_bin_v(iBand+1)-1) ));
        }
        //fftBand_v.clear(); //CF
        //delete[]aux_mem;
    }
...
std::vector<ACMediaTimedFeature*> computeFeaturesBuffered(ACAudio* audio, int mfccNbChannels, int mfccNb, int windowSize, bool extendSoundLimits, int minOctave, int maxOctave){
    vector<string> descList;
    std::vector<ACMediaTimedFeature*> desc;
    descList.push_back("Spectral Centroid");
    descList.push_back("Spectral Spread");
    ...
    if ((mfccNbChannels & (mfccNbChannels-1)) != 0){
        std::cerr << "ACAudioFeatures::computeFeaturesBuffered Number of mfcc channels should be a power of two" << std::endl;
        descList.clear();
        return desc;
    }

    if ((windowSize & (windowSize-1)) != 0){
        std::cerr << "ACAudioFeatures::computeFeaturesBuffered Window size should be a power of two"  << std::endl;
        descList.clear();
        return desc;
    }
    ...
    if ((infile = sf_open (audio->getFileName().c_str(), SFM_READ, &sfinfo)) == NULL)
    {
        std::cerr << "ACAudioFeatures::computeFeaturesBuffered Not able to open input file " << audio->getFileName() << std::endl;
        descList.clear();
        return desc;
    }

    int samplerate = audio->getSampleRate();
    int nchannels = audio->getChannels();
    float n_frames = sfinfo.frames;
    float start = audio->getStart();
    float end = audio->getEnd();

    int last_frame = n_frames;
    if(start<0){
        std::cerr << "ACAudioFeatures::computeFeaturesBuffered start not set" << std::endl;
        sf_close (infile);
        descList.clear();
        return desc;
    }
    else if(start >0){
        int current_frame = sf_seek(infile, (int)(start*samplerate), SEEK_SET) ;
        std::cout << "ACAudioFeatures::computeFeaturesBuffered seeked at " << current_frame << " while " << (int)(start*samplerate) << " was asked." << std::endl;
    }

    if(end<0){
        std::cerr << "ACAudioFeatures::computeFeaturesBuffered end not set" << std::endl;
        sf_close (infile);
        descList.clear();
        return desc;
    }
    else if(end*samplerate>n_frames){
        end = (float)n_frames / samplerate;
        std::cerr << "ACAudioFeatures::computeFeaturesBuffered end beyond sample size" << std::endl;
        sf_close (infile);
        descList.clear();
        return desc;
    }
    else{
        last_frame = (int)((samplerate)*(end-start))-1;
    }
    ...
    // Magic numbers
    int fftSize = windowSize * 2;
    int hopSize = (int)((float)windowSize / 8.0f);
    int sr_hz = 22050;

    // Resampling parameters:
    SF_INFO sfinfoi;
    SF_INFO sfinfoo;
    sfinfoi.samplerate = samplerate;
    sfinfoi.channels = nchannels;
    sfinfoi.seekable = 1;
    sfinfoo.samplerate = sr_hz;
    double srcRatio = (double) sr_hz/ (double) samplerate;

    int bufSize = (int)((float)hopSize/srcRatio*nchannels); //CF floor?
    sfinfoi.frames = bufSize;//CF
    //int outFrames = (int) (length * srcRatio * nchannels + 1);

    float f2b = (float)fftSize/(float)sr_hz;
    float b2f = 1/f2b;

    int n_samples_hop = hopSize;

    int k, m, readcount ;

    long nbFrames = (long)(last_frame*srcRatio/(float)nchannels-windowSize)/hopSize+1;
    int env_fs = (int) sr_hz/hopSize;
    mat melfilter_m;
    mat chromafilter_m;
    ...
    colvec frameFFTabs2_v(fftSize/2);
    colvec prevFrameFFTabs_v;*/
    colvec prevFrameFFTabs_v(fftSize/2);

    mat GDspectrogram=zeros<mat>(nbFrames,fftSize/2);
    rowvec vec1=rowvec(fftSize/2-1);
    rowvec vec2=rowvec(fftSize/2-1);
    colvec ChirpGD=zeros<colvec>(nbFrames);
    colvec inv(windowSize);
    //colvec res=zeros<colvec>(signal_v.n_elem);
    colvec res=zeros<colvec>( sfinfoi.frames*srcRatio/(float)nchannels );//CF
    //colvec f0=zeros<colvec>(nbFrames);
    //colvec ValTmp=zeros<colvec>(nbFrames);
    mat F0_and_Conf(nbFrames,2);
    mat HNR_m=zeros<mat>(nbFrames,4);
    ...
    colvec signal_v;
    signal_v = zeros(windowSize,1);

    std::cout << "ACAudioFeatures::computeFeaturesBuffered resampling " << last_frame << " frames with bufSize " << bufSize << " @ " << samplerate << " Hz and " << nchannels << " channel(s)" << std::endl;
    std::cout << "(int)((float)hopSize/srcRatio)) " << (int)((float)hopSize/srcRatio) << std::endl;
    std::cout << "ACAudioFeatures::computeFeaturesBuffered computing features on "<< last_frame*srcRatio/(float)nchannels <<" frames with hopSize " << hopSize << " @ " << sr_hz << " Hz and 1 channel, windowSize " << windowSize << ", fftSize " << fftSize  << std::endl;

    float progress = 0.0f;
    int t_i = 0;// thumbnail index
    int s = 0;// source index
    int nbufs = 0;
    //float * dataout = new float[hopSize*nchannels];
    //float * datain = new float[(int)((float)hopSize/srcRatio)];

    std::cout << "nbFrames " << nbFrames << std::endl;
    float* buf = new float[bufSize];
    float * dataout = new float[hopSize*nchannels];

    while ( ( (readcount = sf_readf_float (infile, buf, bufSize)) > 0 ) && s<last_frame && last_frame-t_i*bufSize >= bufSize )
    {
        if (readcount!=bufSize)
            std::cerr << "Couldn't read buffer @ nbufs " << nbufs << std::endl;

        //Resample
        if (!dataout)
            std::cerr << "Couldn't initialize buffer " << std::endl;
        int t = resample(buf, &sfinfoi, dataout, &sfinfoo);

        if(nbufs<windowSize){
            for (long i = 0; i < hopSize; i++)
                signal_v(nbufs+i)=dataout[i*nchannels];
            nbufs+=hopSize;
        }
        if(nbufs>=windowSize){
            if(nbufs>=windowSize+hopSize){
                double* aux_mem = new double[windowSize-hopSize];
                colvec temp_v = colvec(aux_mem,windowSize-hopSize);
                temp_v = signal_v.rows(hopSize,windowSize-1);
                signal_v.rows(0,windowSize-hopSize-1) = temp_v;
                //temp_v.clear();
                delete[] aux_mem;
                /*signal_v.shed_rows(0,hopSize-1);
                signal_v.insert_rows(windowSize-hopSize,hopSize);*/

                for (long i = 0; i < hopSize; i++)
                    signal_v(windowSize-hopSize+i)=dataout[i*nchannels];
            }
            nbufs+=hopSize;
            ...
            colvec frame_v = colvec(windowSize);
            colvec frameW_v = colvec(windowSize);
            colvec frameFFTabs_v(fftSize);
            colvec frameFFTabs2_v(fftSize/2);

            prevFrameFFTabs_v.zeros(fftSize/2);

            //time_v(index) = ((double)nbufs+((double)windowSize*(1.0-(double)extendSoundLimits))/2.0)/(double)sr_hz;
            //time_v(index) = ((double)nbufs+((double)windowSize*(1.0))/2.0)/(double)sr_hz;
            time_v(index) = (double)nbufs;

            /*frame_v.set_size(windowSize);
            frameW_h.set_size(windowSize);
            frameW_v.set_size(windowSize);
            frameFFTabs_v.set_size(windowSize);
            frameFFTabs2_v.set_size(fftSize/2);*/

            frame_v = signal_v.rows(0,windowSize-1);
            frameW_v = signal_v.rows(0,windowSize-1)%window_v;
            frameFFTabs_v = abs(fft(frameW_v, fftSize));
            ...
            /*if(nbufs<20*hopSize){
                std::stringstream file;
                file << "frameFFTabs2-" << nbufs << ".txt";
                frameFFTabs2_v.save(std::string(file.str()), arma_ascii);
            }*/

            bool not_finite = false;
            if (!frame_v.is_finite() ){
                std::cerr << "frame_v is not finite "; not_finite = true;
            }
            if (!frameW_v.is_finite() ){
                std::cerr << "frameW_v is not finite "; not_finite = true;
            }
            if (!frameFFTabs_v.is_finite() ){
                std::cerr << "frameFFTabs_v is not finite "; not_finite = true;
            }
            if (!frameFFTabs2_v.is_finite() ){
                std::cerr << "frameFFTabs2_v is not finite "; not_finite = true;
            }
    ...
    ACMediaTimedFeature* mfcc_tf = 0;
    ACMediaTimedFeature* chroma_tf = 0;
    ...
    for (int iDesc = 0; iDesc < descList.size(); iDesc++){
        ...
        {
            if (!sc_v.is_finite() ){
                std::cerr << "sc_v is not finite" << std::endl;
                //exit(1);
            }
            desc.push_back(new ACMediaTimedFeature(conv_to<fcolvec>::from(time_v), conv_to<fmat>::from(sc_v), std::string("Spectral Centroid")));
        }
        ...
        {
            if (!ss_v.is_finite()){
                std::cerr << "ss_v is not finite" << std::endl;
                //exit(1);
            }
            desc.push_back(new ACMediaTimedFeature(conv_to<fcolvec>::from(time_v), conv_to<fmat>::from(ss_v), std::string("Spectral Spread")));
        }
        ...
        {
            if (!sfm_m.is_finite()){
                std::cerr << "sfm_m is not finite" << std::endl;
                //exit(1);
            }
            desc.push_back(new ACMediaTimedFeature(conv_to<fcolvec>::from(time_v), conv_to<fmat>::from(sfm_m), std::string("Spectral Flatness")));
        }
        ...
        {
            if (!sv_v.is_finite()){
                std::cerr << "sv_v is not finite" << std::endl;
                //exit(1);
            }
            desc.push_back(new ACMediaTimedFeature(conv_to<fcolvec>::from(time_v), conv_to<fmat>::from(sv_v), std::string("Spectral Variation")));
        }
        ...
        {
            if (!zcr_v.is_finite()){
                std::cerr << "zcr_v is not finite" << std::endl;
                //exit(1);
            }
            desc.push_back(new ACMediaTimedFeature(conv_to<fcolvec>::from(time_v), conv_to<fmat>::from(zcr_v), std::string("ZCR")));
        }
        ...
        {
            if (!sd_v.is_finite() ){
                std::cerr << "sd_v is not finite" << std::endl;
                //exit(1);
            }
            desc.push_back(new ACMediaTimedFeature(conv_to<fcolvec>::from(time_v), conv_to<fmat>::from(sd_v), std::string("Spectral Decrease")));
        }
        ...
        {
            if (!ener_v.is_finite()){
                std::cerr << "ener_v is not finite" << std::endl;
                //exit(1);
            }
            desc.push_back(new ACMediaTimedFeature(conv_to<fcolvec>::from(time_v), conv_to<fmat>::from(ener_v), std::string("Energy")));
        }
        ...
        {
            if (!loud_v.is_finite()){
                std::cerr << "loud_v is not finite" << std::endl;
                //exit(1);
            }
            desc.push_back(new ACMediaTimedFeature(conv_to<fcolvec>::from(time_v), conv_to<fmat>::from(loud_v), std::string("Loudness")));
        }
        ...
        {
            if (!mfcc_m.is_finite() ){
                std::cerr << "mfcc_m is not finite" << std::endl;
                //exit(1);
            }
            mfcc_tf = new ACMediaTimedFeature(conv_to<fcolvec>::from(time_v), conv_to<fmat>::from(mfcc_m), std::string("MFCC"));
            desc.push_back(mfcc_tf);
        ...
        {
            if (!chroma_m.is_finite() ){
                std::cerr << "chroma_m is not finite" << std::endl;
                //exit(1);
            }
            chroma_tf = new ACMediaTimedFeature(conv_to<fcolvec>::from(time_v), conv_to<fmat>::from(chroma_m), std::string("Chroma"));
            desc.push_back(chroma_tf);
